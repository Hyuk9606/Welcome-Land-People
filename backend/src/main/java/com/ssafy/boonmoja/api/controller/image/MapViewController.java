...
//        if(!mapViewRepo.findByUserAndLoc(userId, loc).getMapType().equals("color")) throw new Exception("It's not color. 잘못된 요청 입니다.");
        if (mapView == null)
            mapView = new MapView(loc, "color", userId);
        else {
            mapView.setMapType("color");
            mapView.setImage(null);
        }
...
//        if (!mapViewRepo.findByUserAndLoc(userId, loc).getMapType().equals("image"))
//            throw new Exception("It's not an image. 잘못된 요청 입니다.");
        log.info("userId = {} , location = {}, image = {}", userId, loc, image);
        MapView mapView = mapViewRepo.findByUserAndLoc(userId, loc);
        if (mapView == null)
            mapView = new MapView(loc, "image", userId);
        else {
            mapView.setMapType("image");
            mapView.setColor(null);
        }
    ...
    @GetMapping("/info")
    public ApiResult<List<MapView>> getMapInfo(@CurrentUser String userId) {
        List<MapView> mapView = mapViewRepo.findByUser(userId);
        return success(mapView);
    }
